...
<?=$this->load->view('includes/header')?>
	...
	<div class="col-sm-offset-1" id='category_option_container'>
		...
		<div id="buttonsAdmin">
			<?=anchor('administrator/create_account', '<button class="btn btn-primary">Create Account</button>')?>
	...
	<div id='search_result_container'>
		...
		<?php if($accountCount > 0) { ?>
			...
			<table id="admin_results" border = "1" cellpadding = "5" cellspacing = "2">
				...
				<tbody>
					...
					<a href="<?=base_url('administrator/edit_account/'.$account->id);?>">
						<tr>
							...
							<td>
							<input type='checkbox'name='users[]' value='<?=$account->id?>' 
							<?php
				...
				<?=anchor('administrator/view_user_profile/'.$this->session->userdata('id'), '<button class="btn btn-success"><span class="glyphicon glyphicon-new-window"></button>')?>
			</table>
		<?php } ?>
	</div>
